  ...
  group('GradientCalculator', () {
    ...
    test('should return a proper gradient vector', () {
      ...
      final gradient = calculator
          .getGradient((MLVector<Float32x4> a, Iterable<MLVector<Float32x4>> vectorArgs, Iterable<double> scalarArgs) {
            final b = (vectorArgs as List<MLVector<Float32x4>>)[0];
            final c = (scalarArgs as List<double>)[0];
            return math.pow(a.dot(b) - c, 2).toDouble();
          }, k, [x], [y], 0.00001)
          .toList()
          .map((double value) => double.parse(value.toStringAsFixed(2)))
          .toList();
